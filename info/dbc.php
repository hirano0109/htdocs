...
class Dbc
{
    ...
    {
        $dbh = $this->dbConnect();
        $stmt = $dbh->prepare('SELECT * FROM info_table WHERE publish_status=:publish_status');
    ...
    public function getABlog($id)
    {
        ...
        $stmt = $dbh->prepare('SELECT * FROM info_table WHERE id = :id');
    ...
    {
        $sql = 'INSERT INTO
            info_table(title, content, category, publish_status)
        VALUES
